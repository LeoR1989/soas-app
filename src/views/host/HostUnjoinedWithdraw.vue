        ...
        <div v-if="selectedMethod">
          ...
          <div class="payment-summary" style="margin-top: 24px;">
            ...
            <div class="flex justify-between items-center" style="margin-top: 8px;">
              <span class="text-body text-secondary">{{ $t('hostWithdraw.convertedAmount') }}</span>
              <span class="text-body num font-bold text-success">≈ {{ diamondsToUSD(parseInt(amountInput)) }} USD</span>
            </div>
            <div class="flex justify-between items-center" style="margin-top: 8px;">
              <span class="text-body text-secondary">{{ $t('hostWithdraw.receivedCurrency') }}</span>
              <span class="text-body font-bold">{{ countryConfig.currency }}</span>
            </div>
          </div>
          <p class="text-caption text-muted" style="margin-top: 12px; line-height: 1.5;">{{ $t('hostWithdraw.feeDisclaimer') }}</p>
      ...
      <div v-if="step === 'result'" class="overlay" @click.self="resetWithdraw">
        <div class="modal-card text-center">
          ...
          <div v-if="resultSuccess && withdrawResult" class="result-details mt-16">
            ...
            <div class="result-row">
              <span class="text-body text-secondary">{{ $t('hostWithdraw.convertedAmount') }}</span>
              <span class="text-body num font-bold">≈ {{ diamondsToUSD(withdrawResult.diamondsDeducted) }} USD</span>
            </div>
            <div class="result-row">
              <span class="text-body text-secondary">{{ $t('hostWithdraw.receivedCurrency') }}</span>
              <span class="text-body font-bold">{{ withdrawResult.receivedCurrency }}</span>
            </div>
            <div class="result-row">
              <span class="text-body text-secondary">{{ $t('hostWithdraw.receivedAmount') }}</span>
              <span class="text-body num text-muted">{{ $t('hostWithdraw.calculating') }}</span>
            </div>
            <div class="result-row">
...
function goBack() {
  router.push('/host/diamond-wallet')
}
