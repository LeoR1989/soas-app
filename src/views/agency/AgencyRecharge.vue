<template>
  <div class="page-container">
    ...
    <div class="withdraw-header px-24">
      <button class="back-btn" @click="$router.push('/agency/dashboard')">‹</button>
    ...
    <div class="balance-card px-24 mt-16">
      <div class="text-caption text-secondary">{{ $t('agencyDashboard.revenuePool') }}</div>
      <div class="balance-row">
        <span class="balance-icon">💎</span>
        <span class="text-hero num">{{ formatNumber(agencyData.current.totalBalance) }}</span>
      </div>
    </div>
    ...
    <Transition name="fade">
      <div v-if="showSuccess" class="overlay" @click.self="resetAndContinue">
        <div class="modal-card text-center">
          ...
          <p class="text-body text-secondary mt-8">{{ $t('recharge.successDesc') }}</p>
...
import { agencyData, DIAMOND_RATE, recentContacts, mockUsers } from '../../mock/data.js'
...
const diamondError = computed(() => {
  ...
  if (val > agencyData.current.totalBalance) return t('recharge.insufficientBalance')
  return ''
})

const canConfirm = computed(() => {
  const val = parseInt(diamondInput.value)
  return selectedUser.value && !isNaN(val) && val > 0 && val <= agencyData.current.totalBalance
})

function setMaxDiamonds() {
  diamondInput.value = String(agencyData.current.totalBalance)
}
...
function doRecharge() {
  ...
  agencyData.current.totalBalance -= diamonds
...
.tab-bar { display: flex; gap: 8px; border-bottom: 1px solid var(--border-subtle); padding-bottom: 0; }
